...
const ModalCreateIncomeExpense = ({ isModalOpen, setIsModalOpen, monthId, eventType }: Props) => {
  ...
  const handleCreateMonth = ({ category, amount }: FormData) => {
    if (!monthId) return;
    ...
    // call createMonth(reqBody)
    // upddate month Income/Expense
  ...
  return (
    <div className='absolute top-0 left-0 z-10 w-screen h-screen '>
      <div className='absolute z-30 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full max-w-xs bg-slate-500 border-2 border-slate-800 rounded-3xl shadow-24 p-4 flex flex-col items-center'>
        <p className='text-gray-200'>{eventType === APP.eventType.income ? 'Add income' : 'Add expense'}</p>
